...
# from keyboards.inline.inline_btn_ru import build_keyboard
from keyboards.default.default_btn_ru import main_btn_ru
import sqlite3



@dp.callback_query_handler(text="payme_card_ru")
async def book_invoice(call: CallbackQuery):
    ...
    print("ishladi 2")
    # pro_text += f"Общая стоимость: {narx} UZS\n"
    ...
    try:
        ...
        await call.message.answer("Уменьшите тип продукта. Мы можем принять только 5 видов продукции одновременно.")
...
async def process_pre_checkout_query(pre_checkout_query: types.PreCheckoutQuery):
    ...
    # check_sell = f"Quyidagi mahsulot sotildi: {pre_checkout_query.provider_data}\n"
    check_sell = f"Telegram user: <a href='tg://user?id={pre_checkout_query.from_user.id}'> {md.quote_html(pre_checkout_query.from_user.first_name)}</a>\n"
